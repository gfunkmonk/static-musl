...
sed -i 's/CFLAGS_BASE = -O2/CFLAGS_BASE = ${BCFLAGS} ${ARCH_FLAGS} ${EXTRA} ${LTO} -fno-PIE -Wno-sign-conversion/g' CPP/7zip/7zip_gcc.mak
sed -i 's/LDFLAGS = -Wall/LDFLAGS = ${BLDFLAGS} ${MOLD} -w -Wl,-s -no-pie/g' CPP/7zip/7zip_gcc.mak
...
make -j\$(nproc) \
  ...
  CC='gcc ${BCFLAGS} ${ARCH_FLAGS} ${EXTRA} ${LTO} -fno-PIE -Wno-sign-conversion' \
  CXX='g++ ${BCFLAGS} ${ARCH_FLAGS} ${EXTRA} ${LTO} -fno-PIE -Wno-sign-conversion'
