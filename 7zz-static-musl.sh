...
sed -i 's/LDFLAGS = -Wall/LDFLAGS = ${BLDFLAGS} ${MOLD} -no-pie/g' CPP/7zip/7zip_gcc.mak
...
make -j\$(nproc) \
  ...
  CC='${CC} ${BCFLAGS} ${ARCH_FLAGS} ${EXTRA} ${LTO} -fno-PIE -Wno-sign-conversion' \
  CXX='${CXX} ${BCFLAGS} ${ARCH_FLAGS} ${EXTRA} ${LTO} -fno-PIE -Wno-sign-conversion'
