...
if [ -n "${RUST_TARGET}" ]; then
  ...
  BUILD_DIR=$(mktemp -d)
  tar xf "distfiles/${RG_TARBALL}" -C "${BUILD_DIR}"

  echo -e "${VIOLET}= Building ripgrep ${RG_VERSION} for ${ARCH} (cross-compilation on host)${NC}"
  pushd "${BUILD_DIR}/ripgrep-${RG_VERSION}/"
  RUSTFLAGS="-C target-feature=+crt-static" \
    cargo zigbuild --release --target "${RUST_TARGET}"
  popd

  package_output "ripgrep" "${BUILD_DIR}/ripgrep-${RG_VERSION}/target/${RUST_TARGET}/release/rg"
  rm -rf "${BUILD_DIR}"
else
