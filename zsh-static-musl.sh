...
./configure CC='clang' --enable-max-jobtable-size=256 --enable-etcdir=/etc/zsh --enable-function-subdirs --with-tcsetpgrp --enable-cap \
  ...
  --with-term-lib="ncursesw tinfo" --enable-ldflags='${BLDFLAGS} ${MOLD} -no-pie' \
  --enable-cflags='${BCFLAGS} ${ARCH_FLAGS} ${EXTRA} ${LTO} -fno-PIE -Wno-unused-variable -Wno-implicit-function-declaration' \
  LDFLAGS='${BLDFLAGS} ${MOLD} -no-pie' PKG_CONFIG='${PKGCFG}' \
  CFLAGS='${BCFLAGS} ${ARCH_FLAGS} ${EXTRA} ${LTO} -fno-PIE -Wno-unused-variable -Wno-implicit-function-declaration'
